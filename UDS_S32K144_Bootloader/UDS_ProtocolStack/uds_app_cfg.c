...
/* Is Read finger print right? */
static uint8 IsReadFingerprintRight(const tUdsAppMsgInfo *m_pstPDUMsg);
...
static void ReadMemoryByIdentifier(struct UDSServiceInfo *i_pstUDSServiceInfo, tUdsAppMsgInfo *m_pstPDUMsg);
...
/* Read bootloader finger print ID */
static const uint8 gs_aReadBootFingerprintID[] = {0x22u, 0xF1u, 0x08u};

/* Read application finger print ID */
static const uint8 gs_aReadAppFingerprintID[] = {0x22u, 0xF1u, 0x09u};
#endif
...
static void ReadMemoryByIdentifier(struct UDSServiceInfo *i_pstUDSServiceInfo, tUdsAppMsgInfo *m_pstPDUMsg)
{
    ...
    /* Is read finger print ID right? */
    if (TRUE == IsReadFingerprintRight(m_pstPDUMsg)) //
    {
        if ((m_pstPDUMsg->aDataBuf[1] == 0xF1u) && (m_pstPDUMsg->aDataBuf[2]) == 0x08u )
        {
            /* Do read finger print */
            m_pstPDUMsg->aDataBuf[0u] = i_pstUDSServiceInfo->SerNum + 0x40u;
            m_pstPDUMsg->aDataBuf[1u] = 0xF1;
            m_pstPDUMsg->aDataBuf[2u] = 0x08;
            m_pstPDUMsg->aDataBuf[3u] = 0x01; //ASC-II : "1"
            m_pstPDUMsg->aDataBuf[4u] = 0x2E; //ASC-II : "."
            m_pstPDUMsg->aDataBuf[5u] = 0x00; //ASC-II : "0"
        ...
        else
        {
            ...
            m_pstPDUMsg->aDataBuf[0u] = i_pstUDSServiceInfo->SerNum + 0x40u;
            m_pstPDUMsg->aDataBuf[1u] = 0xF1;
            m_pstPDUMsg->aDataBuf[2u] = 0x09;
            m_pstPDUMsg->aDataBuf[3u] = 0x01; //ASC-II : "1"
            m_pstPDUMsg->aDataBuf[4u] = 0x2E; //ASC-II : "."
            m_pstPDUMsg->aDataBuf[5u] = 0x01; //ASC-II : "0"
            m_pstPDUMsg->xDataLen = 6u;
        }
    }
    else
...
static void RequestDownload(struct UDSServiceInfo *i_pstUDSServiceInfo, tUdsAppMsgInfo *m_pstPDUMsg)
{
    ...
    if (TRUE == Ret)
    {
        ...
        m_pstPDUMsg->aDataBuf[2u] = 0x82u; /* TODO Bootloader: #05 Ϊ���� ZCANPRO����ԭ���� 0x80 ��Ϊ 0x82 */
        m_pstPDUMsg->xDataLen = 3u;
...
{
    uint8 Index = 0u;
    uint8 ReadFingerprintIdLen = 0u;
    uint8 *pDestFingerprintID = NULL_PTR;
    ASSERT(NULL_PTR == m_pstPDUMsg);

    if ((m_pstPDUMsg->aDataBuf[1] == 0xF1u) && (m_pstPDUMsg->aDataBuf[2]) == 0x08u )
    {
    	pDestFingerprintID = (uint8 *)&gs_aReadBootFingerprintID[0];
        ReadFingerprintIdLen = sizeof(gs_aReadBootFingerprintID);
    }
    else if ((m_pstPDUMsg->aDataBuf[1] == 0xF1u) && (m_pstPDUMsg->aDataBuf[2] == 0x09u))
    {
    	pDestFingerprintID = (uint8 *)&gs_aReadAppFingerprintID[0];
        ReadFingerprintIdLen = sizeof(gs_aReadAppFingerprintID);
    }
    else
    {
    	return FALSE;
    ...
    if (m_pstPDUMsg->xDataLen < ReadFingerprintIdLen)
    {
        return FALSE;
    }


    while (Index < ReadFingerprintIdLen)
    {
        if ((m_pstPDUMsg->aDataBuf[Index]) != pDestFingerprintID[Index])
        {
            return FALSE;
        }
